...
if __name__ == "__main__":
    ...
    LOG_PATH = './models_pretrained/log'
    writer = SummaryWriter(LOG_PATH)
    filename = '{}/checkpoint'.format(SAVE_PATH)  # 文件扩展名在保存时添加
    ...
    model_ft.load_state_dict(torch.load(PRETRAINED_PATH),  strict = False)
    # 修改模型
    model_ft.conv1 = nn.Conv2d(3, 64, 3, stride=1, padding=1, bias=False)  # 首层改成3x3卷积核
    model_ft.maxpool = nn.MaxPool2d(1, 1, 0)  #通过1x1的池化核让池化层失效
